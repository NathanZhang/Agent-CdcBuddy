...
> **执行时间**：2026-09-24 20:27:02 ~ 2026-09-24 20:27:03  
> **测试结果**：**19/19 全部通过 (100.0%)** | **总耗时**：1.26s  
> **断言通过率**：**324/324 (100%)**  
...
| :---: | :--- | :--- | :--- | :--- | :---: | :---: |
| **REQ-23** | 种群动态模型 | 种群动态分析 | 全省蚊媒密度随气温变化的季节消长规律及未来3个月ARIMA时序预测 | r2Score: 0.65, historicalMonths: 40 | 110.72ms | ✅ **PASS** |
| **REQ-24** | 种群识别模型 | 种群动态分析 | 郑州市蚊类优势种群构成比（白纹伊蚊 vs 淡色库蚊）识别与 K-Means 聚类 | dominantSpecies: 淡色(致倦)库蚊, shannonWienerIndex: 1.407 | 394.1ms | ✅ **PASS** |
| **REQ-25** | 抗药性预测模型 | 抗药性评估 | 输入杀虫剂类型与监测点数据，预测病媒生物耐药等级（敏感/中抗/高抗）并推荐消杀用药处方 | evaluatedRecords: 15, topSpecies: 家蝇 | 2.29ms | ✅ **PASS** |
| **REQ-26** | 病原携带风险评估模型 | 病原携带风险 | 整合病媒 PCR 检测结果与宿主分布，通过 Apriori 关联规则挖掘高风险病原组合 | highRiskItemsCount: 36, associationRulesCount: 8 | 189.55ms | ✅ **PASS** |
| **REQ-27** | 动态预警分析 | 动态预警响应 | 多维度预警触发与 IDW 空间插值连续热力图生成，支持地市与区县下钻 | totalAlerts: 30, redAlerts: 1 | 27.39ms | ✅ **PASS** |
| **REQ-28** | 预警推送信息 | 动态预警响应 | 按风险等级自动分类预警，生成通知推送卡片与依据，并持久化写入业务库 | persistedEventId: TEST-ALERT-1790252823, pushStatus: SENT | 25.29ms | ✅ **PASS** |
| **REQ-29** | 处置闭环信息 | 动态预警响应 | 智能生成消杀处置工单、关联三步消杀规程、流转跟踪并通过 BI 复测闭环核销 | ticketId: TICKET-TEST-1790252823, statusTransition: IN_PROGRESS -> RESOLVED | 1.48ms | ✅ **PASS** |
| **REQ-30** | 密度预测模型 | 风险预测评估 | 融合气象数据与地理生境，通过 GBDT 梯度提升回归模型预测未来 1-2 个月密度及因子权重 | city: 郑州市, predictedDensity: 1.37 | 39.65ms | ✅ **PASS** |
| **REQ-31** | 传播风险评估模型 | 风险预测评估 | 构建'病媒密度 × 病原携带率 × 人群暴露指数'关联数学模型，量化传染病传播风险 (0-100) | riskScore: 44.2, riskLevel: 中等传播风险 (Yellow) | 5.61ms | ✅ **PASS** |
| **REQ-32** | 抗药性演化预测 | 风险预测评估 | 基于历史数据与用药频率，贝叶斯/马尔可夫网络预测 1 年内耐药基因频率演化与突变暴发预警 | evolutionYears: ['2020', '2021', '2022', '2023', '2024', '2025', '2026 (预测)', '2027 (预测)'], kdrGeneFrequency: [0.66, 0.55, 0.55, 0.55, 0.55, 0.53, 0.64, 0.74] | 0.98ms | ✅ **PASS** |
| **REQ-33** | 自然语言问答(NLQ) | 智能问答 | CDC 知识库全文检索与标准条文精准匹配（GB/T 标准、物种鉴别、应急消杀阈值） | queriesTested: 4, hitsCount: 4 | 0.37ms | ✅ **PASS** |
| **REQ-34** | 自动生成专题报告 | 专题报告 | 自动提取多维时空数据、生成图表文字综述（四段式报告）并持久化归档至业务库 | reportId: REP-TEST-1790252823, title: 郑州市 2024年病媒生物监测与风险预警专项报告 (测试版) | 3.09ms | ✅ **PASS** |
| **REQ-35** | 移动端智能辅助(API) | 移动端接口 | 移动端 API 仿真：拍照物种 AI 识别、现场记录提交与气象生境数据逻辑质控规则校验 | submissionId: MOB-SUB-1790252823, aiRecognizedSpecies: 白纹伊蚊 | 1.01ms | ✅ **PASS** |
| **EXT-01** | 对话式自定义创建技能 | 自定义技能 | 对话式动态编译 SQL 聚合逻辑、生成专属图表卡片并持久化注册新技能 | customSkillId: test_custom_skill_top5_analysis, skillName: 郑州市主要病媒物种捕获TOP5分析 | 25.82ms | ✅ **PASS** |
| **EXT-02** | SaTScan ➔ K-Means ➔ LSTM 多步科学计算流水线 | 复杂计算编排 | 泊松时空扫描+亚群画像+LSTM带状外推 | targetTime: 2022年3月, clustersCount: 3 | 88.48ms | ✅ **PASS** |
| **EXT-03** | 后台常驻数据分析智能体 | 智能体守护 | 提示词策略注入+自发预警+队列推送 | cycleTime: 2026-09-24 20:27:03, detectedAnomalies: 10 | 24.64ms | ✅ **PASS** |
| **EXT-04** | 独立原子技能 · SaTScan 空间泊松扫描 | 空间聚集扫描 | 单独调用 SaTScan 空间泊松时空扫描，输出纯 GIS 扫描图层、显著聚集簇与 LLR/RR 统计量 | title: 河南省 2022年6月 蚊类 SaTScan 空间泊松时空扫描研判报告, clustersCount: 3 | 24.93ms | ✅ **PASS** |
| **EXT-05** | 独立原子技能 · LSTM 深度时序预测 | 深度时序预测 | 单独运行 LSTM 递归神经网络，滚动预测指定地市未来 7 天日级密度走势与 95% 置信带 | targetCity: 信阳市, forecastHorizon: 7 天 | 52.82ms | ✅ **PASS** |
| **EXT-06** | 通用可编排 LangGraph 多技能动态工作流 | 通用工作流 | 动态编排 3 项跨领域技能 (SaTScan 空间扫描 ➔ 病原 PCR 关联 ➔ 自动消杀派单)，共享上下文自动传递 | workflowName: SaTScan空间扫描 ➔ 病原关联挖掘 ➔ 自动消杀派单 协同流, totalSteps: 3 | 239.72ms | ✅ **PASS** |
...
```json
{
  "highRiskItemsCount": 36,
  "associationRulesCount": 8,
  "topRiskPair": "淡色库蚊 + 登革病毒 (DENV)",
  "positivityRate": "0.0%",
...
```json
{
  "persistedEventId": "TEST-ALERT-1790252823",
  "pushStatus": "SENT",
...
```json
{
  "ticketId": "TICKET-TEST-1790252823",
...
```json
{
  "submissionId": "MOB-SUB-1790252823",
...
```json
{
  "customSkillId": "test_custom_skill_top5_analysis",
...
### 【EXT-02】SaTScan ➔ K-Means ➔ LSTM 多步科学计算流水线
- **功能分类**：复杂计算编排
- **测试场景**：泊松时空扫描+亚群画像+LSTM带状外推
- **执行耗时**：`88.48 ms`
- **断言结果**：`8/8`
- **关键输出指标**：
```json
{
  ...
  "clustersCount": 3,
  "primaryCity": "漯河市",
  "primaryLLR": 15361.517,
  "kmeansSubgroups": 3,
  "requiresHilReview": true
}
```
- **断言日志详情**：
```text
  [PASS] SaTScan ➔ K-Means ➔ LSTM 多步流水线成功执行
  [PASS] SaTScan 成功扫描出 3 个空间聚集区
  [PASS] 一级聚集区 LLR=15361.517 > 0
  [PASS] 一级聚集区相对危险度 RR=4.17 >= 1.0
  [PASS] K-Means 成功完成 3 类生态亚群画像与消杀配方赋能
  [PASS] LSTM 递归神经网络成功完成未来 7 天密度外推
  [PASS] 成功为所有高危热点城市生成带状 95% 置信区间预测曲线
  [PASS] 完整记录 LangGraph 状态图 5 个节点的审计日志
```

### 【EXT-03】后台常驻数据分析智能体
- **功能分类**：智能体守护
- **测试场景**：提示词策略注入+自发预警+队列推送
- **执行耗时**：`24.64 ms`
- **断言结果**：`7/7`
- **关键输出指标**：
```json
{
  "cycleTime": "2026-09-24 20:27:03",
  "detectedAnomalies": 10,
  "generatedAlerts": 3,
  "sampleAlertId": "ALERT-202609-916",
  "pushedChannel": "cdc_alert_stream"
}
```
- **断言日志详情**：
```text
  [PASS] 后台常驻智能体巡检周期成功执行
  [PASS] 成功检测到 10 处异常密度突增点
  [PASS] 自发研判生成并持久化 3 起分级预警事件
  [PASS] 自动生成合规预警编码: ALERT-202609-916
  [PASS] 正确量化预警等级: red
  [PASS] 预警事件已真实写入业务持久化数据库 biz_early_warning_events
  [PASS] 成功将预警事件推送到 cdc_alert_stream 异步消息通道
```

### 【EXT-04】独立原子技能 · SaTScan 空间泊松扫描
- **功能分类**：空间聚集扫描
- **测试场景**：单独调用 SaTScan 空间泊松时空扫描，输出纯 GIS 扫描图层、显著聚集簇与 LLR/RR 统计量
- **执行耗时**：`24.93 ms`
- **断言结果**：`6/6`
- **关键输出指标**：
```json
{
  "title": "河南省 2022年6月 蚊类 SaTScan 空间泊松时空扫描研判报告",
  "clustersCount": 3,
  "primaryCity": "漯河市",
  "primaryDistrict": "源汇区",
  "primaryRR": 4.95,
...
- **断言日志详情**：
```text
  [PASS] 独立 SaTScan 空间扫描执行成功
  [PASS] 确认处于 standalone_satscan 独立研判模式
  [PASS] 绑定独立生成式组件 SatScanSpatialCard
  [PASS] 成功扫描识别 3 个空间聚集簇
  [PASS] 一类核心聚集区 LLR=1335.077 > 0
  [PASS] 相对危险度 RR=4.95 >= 1.0
```

### 【EXT-05】独立原子技能 · LSTM 深度时序预测
- **功能分类**：深度时序预测
- **测试场景**：单独运行 LSTM 递归神经网络，滚动预测指定地市未来 7 天日级密度走势与 95% 置信带
- **执行耗时**：`52.82 ms`
- **断言结果**：`7/7`
- **关键输出指标**：
```json
...
- **断言日志详情**：
```text
  [PASS] 独立 LSTM 深度时序预测执行成功
  [PASS] 确认处于 standalone_lstm 独立时序外推模式
  [PASS] 绑定独立生成式组件 LSTMPredictorCard
  [PASS] 包含信阳市日级预测序列
  [PASS] 完整输出 7 天滚动日级序列 (实际 7 天)
  [PASS] 置信区间上界 ci_upper >= 点估计密度
  [PASS] 置信区间下界 ci_lower <= 点估计密度
```

### 【EXT-06】通用可编排 LangGraph 多技能动态工作流
- **功能分类**：通用工作流
- **测试场景**：动态编排 3 项跨领域技能 (SaTScan 空间扫描 ➔ 病原 PCR 关联 ➔ 自动消杀派单)，共享上下文自动传递
- **执行耗时**：`239.72 ms`
- **断言结果**：`6/6`
- **关键输出指标**：
```json
...
- **断言日志详情**：
```text
  [PASS] 动态组合 LangGraph 工作流执行成功
  [PASS] 成功流转全部 3 个动态编排节点
  [PASS] 绑定通用多步骤工作流看板 ComposableWorkflowCard
  [PASS] 三步产物全部在 step_results 中完整沉淀
  [PASS] Step 1 识别的高危城市成功传递到 shared_context 供 Step 2/3 消费
  [PASS] 完整记录动态 DAG 执行流审计日志 (4 条)
```
