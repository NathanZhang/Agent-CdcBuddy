...
> **执行时间**：2026-08-12 14:18:55 ~ 2026-08-12 14:18:56  
> **测试结果**：**19/19 全部通过 (100.0%)** | **总耗时**：1.23s  
> **断言通过率**：**324/324 (100%)**  
...
| :---: | :--- | :--- | :--- | :--- | :---: | :---: |
| **REQ-23** | 种群动态模型 | 种群动态分析 | 全省蚊媒密度随气温变化的季节消长规律及未来3个月ARIMA时序预测 | r2Score: 0.65, historicalMonths: 40 | 97.21ms | ✅ **PASS** |
| **REQ-24** | 种群识别模型 | 种群动态分析 | 郑州市蚊类优势种群构成比（白纹伊蚊 vs 淡色库蚊）识别与 K-Means 聚类 | dominantSpecies: 淡色(致倦)库蚊, shannonWienerIndex: 1.407 | 413.33ms | ✅ **PASS** |
| **REQ-25** | 抗药性预测模型 | 抗药性评估 | 输入杀虫剂类型与监测点数据，预测病媒生物耐药等级（敏感/中抗/高抗）并推荐消杀用药处方 | evaluatedRecords: 15, topSpecies: 家蝇 | 2.22ms | ✅ **PASS** |
| **REQ-26** | 病原携带风险评估模型 | 病原携带风险 | 整合病媒 PCR 检测结果与宿主分布，通过 Apriori 关联规则挖掘高风险病原组合 | highRiskItemsCount: 34, associationRulesCount: 8 | 186.16ms | ✅ **PASS** |
| **REQ-27** | 动态预警分析 | 动态预警响应 | 多维度预警触发与 IDW 空间插值连续热力图生成，支持地市与区县下钻 | totalAlerts: 30, redAlerts: 1 | 27.22ms | ✅ **PASS** |
| **REQ-28** | 预警推送信息 | 动态预警响应 | 按风险等级自动分类预警，生成通知推送卡片与依据，并持久化写入业务库 | persistedEventId: TEST-ALERT-1786515535, pushStatus: SENT | 25.46ms | ✅ **PASS** |
| **REQ-29** | 处置闭环信息 | 动态预警响应 | 智能生成消杀处置工单、关联三步消杀规程、流转跟踪并通过 BI 复测闭环核销 | ticketId: TICKET-TEST-1786515535, statusTransition: IN_PROGRESS -> RESOLVED | 1.25ms | ✅ **PASS** |
| **REQ-30** | 密度预测模型 | 风险预测评估 | 融合气象数据与地理生境，通过 GBDT 梯度提升回归模型预测未来 1-2 个月密度及因子权重 | city: 郑州市, predictedDensity: 1.37 | 39.83ms | ✅ **PASS** |
| **REQ-31** | 传播风险评估模型 | 风险预测评估 | 构建'病媒密度 × 病原携带率 × 人群暴露指数'关联数学模型，量化传染病传播风险 (0-100) | riskScore: 44.2, riskLevel: 中等传播风险 (Yellow) | 6.34ms | ✅ **PASS** |
| **REQ-32** | 抗药性演化预测 | 风险预测评估 | 基于历史数据与用药频率，贝叶斯/马尔可夫网络预测 1 年内耐药基因频率演化与突变暴发预警 | evolutionYears: ['2020', '2021', '2022', '2023', '2024', '2025', '2026 (预测)', '2027 (预测)'], kdrGeneFrequency: [0.66, 0.55, 0.55, 0.55, 0.55, 0.53, 0.64, 0.74] | 1.04ms | ✅ **PASS** |
| **REQ-33** | 自然语言问答(NLQ) | 智能问答 | CDC 知识库全文检索与标准条文精准匹配（GB/T 标准、物种鉴别、应急消杀阈值） | queriesTested: 4, hitsCount: 4 | 0.28ms | ✅ **PASS** |
| **REQ-34** | 自动生成专题报告 | 专题报告 | 自动提取多维时空数据、生成图表文字综述（四段式报告）并持久化归档至业务库 | reportId: REP-TEST-1786515535, title: 郑州市 2024年病媒生物监测与风险预警专项报告 (测试版) | 2.52ms | ✅ **PASS** |
| **REQ-35** | 移动端智能辅助(API) | 移动端接口 | 移动端 API 仿真：拍照物种 AI 识别、现场记录提交与气象生境数据逻辑质控规则校验 | submissionId: MOB-SUB-1786515535, aiRecognizedSpecies: 白纹伊蚊 | 0.91ms | ✅ **PASS** |
| **EXT-01** | 对话式自定义创建技能 | 自定义技能 | 对话式动态编译 SQL 聚合逻辑、生成专属图表卡片并持久化注册新技能 | customSkillId: test_custom_skill_top5_analysis, skillName: 郑州市主要病媒物种捕获TOP5分析 | 25.85ms | ✅ **PASS** |
| **EXT-02** | SaTScan ➔ K-Means ➔ LSTM 多步科学计算流水线 | 复杂计算编排 | 泊松时空扫描+亚群画像+LSTM带状外推 | targetTime: 2022年3月, clustersCount: 3 | 84.05ms | ✅ **PASS** |
| **EXT-03** | 后台常驻数据分析智能体 | 智能体守护 | 提示词策略注入+自发预警+队列推送 | cycleTime: 2026-08-12 14:18:56, detectedAnomalies: 10 | 22.6ms | ✅ **PASS** |
| **EXT-04** | 独立原子技能 · SaTScan 空间泊松扫描 | 空间聚集扫描 | 单独调用 SaTScan 空间泊松时空扫描，输出纯 GIS 扫描图层、显著聚集簇与 LLR/RR 统计量 | title: 河南省 2022年6月 蚊类 SaTScan 空间泊松时空扫描研判报告, clustersCount: 3 | 23.32ms | ✅ **PASS** |
| **EXT-05** | 独立原子技能 · LSTM 深度时序预测 | 深度时序预测 | 单独运行 LSTM 递归神经网络，滚动预测指定地市未来 7 天日级密度走势与 95% 置信带 | targetCity: 信阳市, forecastHorizon: 7 天 | 27.42ms | ✅ **PASS** |
| **EXT-06** | 通用可编排 LangGraph 多技能动态工作流 | 通用工作流 | 动态编排 3 项跨领域技能 (SaTScan 空间扫描 ➔ 病原 PCR 关联 ➔ 自动消杀派单)，共享上下文自动传递 | workflowName: SaTScan空间扫描 ➔ 病原关联挖掘 ➔ 自动消杀派单 协同流, totalSteps: 3 | 243.74ms | ✅ **PASS** |
...
```json
{
  "persistedEventId": "TEST-ALERT-1786515535",
  "pushStatus": "SENT",
...
```json
{
  "ticketId": "TICKET-TEST-1786515535",
...
```json
{
  "submissionId": "MOB-SUB-1786515535",
...
```json
{
  "cycleTime": "2026-08-12 14:18:56",
  "detectedAnomalies": 10,
  "generatedAlerts": 3,
  "sampleAlertId": "ALERT-202608-904",
...
```text
  ...
  [PASS] 自动生成合规预警编码: ALERT-202608-904
  [PASS] 正确量化预警等级: red
